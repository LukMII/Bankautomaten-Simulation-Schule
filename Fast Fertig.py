...
def display_main_menu():
    ...
    def show_change_pin_window():
        ...
        pin_change_window.configure(bg=background_color)  # Hintergrundfarbe einstellen
...
def check_pin(pin_entry):
    ...
    global limit
    if pin_entry == pin:
        generate_security_key()  # Sicherheitsschlüssel generieren
    else:
        counter += 1
        if counter >= limit:
...
def generate_security_key():
    global security_key
    security_key = random.randint(1000, 9999)  # Vierstellige Zufallszahl
