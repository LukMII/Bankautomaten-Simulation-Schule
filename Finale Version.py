...
def check_pin(entered_pin, pin_window):
    global counter
    global limit
    if entered_pin == pin:
        ...
        counter += 1
        if counter >= limit:
            show_locked_message(pin_window)
        else:
            messagebox.showwarning("Warnung", f"Falscher PIN! Noch {limit - counter} Versuche übrig.")
            pin_entry.delete(0, tk.END)

# Funktion zur Anzeige der Nachricht bei gesperrter Karte
def show_locked_message(pin_window):
    pin_window.destroy()
    locked_window = tk.Tk()
    locked_window.title("Karte gesperrt")
    locked_window.configure(bg=background_color)

    label = tk.Label(locked_window, text="Karte gesperrt! Bitte kontaktieren Sie 0800 000 111", font=("Arial", 12), bg=background_color, fg=text_color)
    label.pack(pady=20)

    def call_support():
        generate_security_key()
        locked_window.destroy()

    call_button = tk.Button(locked_window, text="Anrufen", font=("Arial", 12), command=call_support, bg=button_color, fg=text_color)
